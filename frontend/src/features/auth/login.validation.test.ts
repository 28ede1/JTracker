...
// same rules. See that file for the vitest API and for how the password tests
// are laid out. Testing them separately anyway is deliberate: the two files are
// free to drift apart, and the day log-in stops demanding a special character
// the sign-up tests must not start failing.
//
// Worth knowing while reading these: checking password strength on log-in is a
// choice, not a requirement. Strength rules exist to stop a weak password being
// created, which has already happened by the time someone logs in. The cost is
// that an account whose password predates a rule can no longer get in through
// this form, since it is refused before Supabase is ever asked. It is safe here
// because sign-up enforces the identical rules, so no account can exist that
// log-in would lock out.
//
...
// Password rules
//
// One test per rule, laid out as in signup.validation.test.ts.
// ---------------------------------------------------------------------------
...
describe('logInRules, password', () => {
  ...
  // These are the strings a user reads when a log-in is refused before it is
  // even attempted, which is the case that only exists on this side.
